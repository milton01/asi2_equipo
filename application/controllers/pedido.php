...
class pedido extends CI_Controller {
    ...
    public function index() { 
        ...
        $datajs['files'] = array('j1', 'j2', 'j3');
        $this->template->write('title', 'Altuve | Pedidos');
        $this->template->write('header', 'Pedidos');
        $this->template->write_view('menu', 'menu/menu_admin');
        $this->template->write_view('content', 'pedido/listar', $data);
        $this->template->write_view('scripts', 'master-scripts', $datajs);
    ...
    public function despachar($pd_id) {        
        ...
        //Se carga la vista con plantilla        
        $datajs['files'] = array('j1', 'j2', 'j3');
        $this->template->write('title', 'Altuve | Despachar pedido');
        $this->template->write('header', 'Despachar pedido');
        $this->template->write_view('menu', 'menu/menu_admin');
        $this->template->write_view('content', 'pedido/despachar', $data);
        $this->template->write_view('scripts', 'master-scripts', $datajs);
        $this->template->render();
    }
    ...
    public function rechazar($pd_id) {        
        ...
        //Se carga la vista con plantilla        
        $datajs['files'] = array('j1', 'j2', 'j3');
        $this->template->write('title', 'Altuve | Rechazar pedido');
        $this->template->write('header', 'Rechazar pedido');
        $this->template->write_view('menu', 'menu/menu_admin');
        $this->template->write_view('content', 'pedido/rechazar', $data);
        $this->template->write_view('scripts', 'master-scripts', $datajs);
        $this->template->render();
    }
